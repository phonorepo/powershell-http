...
using System.Text;
    ...
    public class HttpFileServer : IDisposable
    {
        ...
        private void requestWait(IAsyncResult ar)
        {
            if (!http.IsListening)
                return;
            System.Net.HttpListenerContext c = http.EndGetContext(ar);
        ...
        private void returnDirContents(HttpListenerContext context, string dirPath)
        {
            ...
            {
                sw.WriteLine("<html>");
                sw.WriteLine("<head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"><style>body {zoom: " + 3 + "; -moz-transform: scale(" + 3 + "); -moz-transform-origin: 0 0;}</style></head>");
                sw.WriteLine("<body><ul>");

                string[] dirs = Directory.GetDirectories(dirPath);
                foreach (string d in dirs)
                {
                    string link = d.Replace(RootPath, "").Replace('\\', '/');
                ...
                {
                    string link = f.Replace(RootPath, "").Replace('\\', '/');
        ...
        private static void returnFile(HttpListenerContext context, string filePath)
        {
            ...
            try
            {
                ...
                using (System.IO.FileStream fs = File.OpenRead(filePath))
                {
